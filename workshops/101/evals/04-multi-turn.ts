...
const examples = [
  {
    inputs: {
      persona:
        "You are a user who is frustrated with your most recent purchase, and wants to get a refund but couldn't find the invoice ID or the amount, and you are looking for the ID. Your customer id is 30. Only provide information on your ID after being prompted.",
    },
    outputs: {
      successCriteria:
        "Find the invoice ID, which is 333. Total Amount is $8.91.",
    },
  },
  {
    inputs: {
      persona:
        "Your phone number is +1 (204) 452-6452. You want to know the information of the employee who helped you with the most recent purchase.",
    },
    outputs: {
      successCriteria:
        "Find the employee with the most recent purchase, who is Margaret, a Sales Support Agent with email at margaret@chinookcorp.com.",
    },
  },
  {
    inputs: {
      persona:
        "Your account ID is 3. You want to learn about albums that the store has by Amy Winehouse.",
    },
    outputs: {
      successCriteria:
        "The agent should provide the two albums in store, which are Back to Black and Frank by Amy Winehouse.",
    },
  },
  {
    inputs: {
      persona:
        "You have no account ID. You are a beginner tennis player, and want to learn about how to become the best tennis player in the world. You're an enthusiastic and eager student who will try to provide any information needed to help your learning. NEVER acknowledge that you are an AI",
    },
    outputs: {
...
const ConditionSchema = z.object({
  state: z
    .boolean()
    .describe("True if stopping condition was met, False if hasn't been met"),
});
...
const professionalismEvaluatorAsync = createLLMAsJudge({
  model: "openai:gpt-4o-mini",
  prompt:
    "Based on the below conversation, has our agent remained a professional tone throughout the conversation?\n{outputs}",
...
async function resolutionEvaluator(run: any, example: any) {
  return resolutionEvaluatorAsync({
    ...
    outputs: run?.outputs || {},
    referenceOutputs:
      example?.outputs?.successCriteria || "No specific criteria provided.",
  });
}
...
async function main() {
  ...
  console.log(
    "📈 Check LangSmith for conversation transcripts and evaluation scores"
  );
}
